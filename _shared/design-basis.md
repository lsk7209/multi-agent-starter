...
- **D9 knot 배포의 결정성·opt-in 구조** = knot(벤더중립 standalone 지식 vault, repo 밖 산출물)를 generator의 **단일 opt-in 번들**로 배포한다. 결정성 근거 4개: ① `--with-knot` opt-in 분리 — 기본 init은 번들 템플릿만 복사(모델 창작 0)라는 init.py 결정성을 보존하고, knot 지원은 플래그가 켜질 때만 더해진다. ② 자동층(관리블록)은 명시 마커(`<!-- knot:start/end -->`)로 래핑 — generator가 멱등 재생성·제거 가능(손편집 금지, drift 안전). ③ 능동층(knot 스킬)은 번들 `knot-skill/SKILL.md`를 호스트 스킬 경로로 **바이트 그대로 복사**(claude=`.claude/skills/knot/`, codex=`.codex/skills/`, antigravity=`.antigravity/skills/` — 후자 둘은 워크스페이스-로컬 자동로드 경로 미확정이라 best-effort, 한계는 스킬 §2 "vault 정본 직접 지시" 폴백이 흡수). 스킬·관리블록은 절차 본문을 복붙하지 않고 vault 정본(`prompts/`·`scripts/`)을 가리키는 얇은 shim — 절차 수정은 vault 한 곳. ④ 미설치/`$KNOT_VAULT` 미설정이면 완전 no-op. **INV(system-invariants) 대상 아님**(D8과의 비대칭): INV는 *모든 생성물의 기본상태*가 만족해야 하는 불변식인데, knot은 기본 생성물이 아니라 opt-in 추가물이라 기본상태 검사로 표현 불가하다. 회귀보호는 validate C10(관리블록 정본 일치)·C11(자동·능동층 결합 불변식: 관리블록 ⟺ 스킬 파일, 둘 다 부재=PASS)과 tests/test_generate.py(3 flavor × 주입·복사·멱등·기본부재·C11 네거티브)가 담당. 트랙 A 카파시 4원칙(D8)은 모든 flavor 기본 생성물에 박히므로 INV12로 보호하지만, knot은 opt-in이므로 INV가 아니라 opt-in 전용 체크로 보호하는 게 정합적. (2026-06-13, tasks/karpathy-wiki-upgrade/ 트랙 B 배포 P1~P3. 정본 spec=artifacts/knot-distribution-design.md)
