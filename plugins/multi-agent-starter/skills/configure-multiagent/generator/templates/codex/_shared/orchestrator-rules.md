...
- 작업 전 필요한 스킬을 `있음(사용) / 부족(개선) / 없음(개발)`으로 분류한다.
...
작업 시작 전 Orchestrator가 필요한 스킬을 점검한다.

1. 필요한 스킬을 판단한다.
2. 보유 스킬이 있으면 사용한다. 후보: `ait-builder`, `blog-optimizer`, `title-generator`, `adsense-optimizer`.
3. 부족하면 개선 후보를 만들고, 없으면 신규 개발 후보를 만든다.
4. 신규·개선 스킬은 `codex-critic` 검증을 통과해야 한다.
5. 스킬 풀 영구 등록은 비가역 행동이므로 사용자 승인 전에는 하지 않는다.
6. worker나 외부 출력은 never-trust-upstream 원칙으로 재검증한다. 생성된 스킬을 무비판 등록하지 않는다.
...
- 개선형 작업(기존 콘텐츠 품질, 애드센스 승인 가능성, SEO 품질 개선)은 Act-Observe-Decide 루프를 켠다.
- 창조형 0→1 개발은 루프를 끄거나 최소화한다.
- 종료 조건은 품질 충족, 최대 3회, 토큰·시간 상한 도달 중 하나다.
- 상한에 도달하면 멈추고 사용자에게 남은 차단 요소와 다음 선택지를 보고한다.
